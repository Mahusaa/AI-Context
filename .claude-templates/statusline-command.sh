...
if [ $TOTAL_CHANGES -eq 0 ]; then
  ...
  [ $MODIFIED -gt 0 ] && [ -n "$DETAILS" ] && DETAILS="$DETAILS, ${MODIFIED} modified" || DETAILS="${MODIFIED} modified"
  [ $UNTRACKED -gt 0 ] && [ -n "$DETAILS" ] && DETAILS="$DETAILS, ${UNTRACKED} untracked" || DETAILS="${UNTRACKED} untracked"

  CHANGES_TEXT="$CHANGES_TEXT ($DETAILS)"
fi
